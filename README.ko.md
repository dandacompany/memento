...
[빠른 설치](#빠른-설치) · [프로바이더 매트릭스](#프로바이더-매트릭스) · [CLI](#cli) · [설정](#설정) · [안전 모델](#안전-모델) · [개발](#개발)
...
```text
src/
  adapters/      프로바이더별 파일 reader/writer
  cli/           commander CLI와 command handler
...
현재 npm release: `0.1.2`.

v0.1 라인은 다음을 포함합니다.

- 6개 provider adapter
- `project`, `project-local`, `global` memory tier
- `init`, `status`, `sync`, `watch`, `diff`, `restore`, `global` 명령
- Claude Code skill packaging
- 자동 백업과 restore
- Linux, macOS, Windows에서 Node 18, 20, 22 CI

MVP 이후 방향:

- Aider, Continue.dev 같은 추가 provider adapter
- 실제 provider 설치 환경을 대상으로 한 더 풍부한 integration test
- orphan memory file이 많은 저장소의 first-run 안내 개선
- 선택적 editor/IDE integration
